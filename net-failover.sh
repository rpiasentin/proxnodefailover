...
dhclient_stop() {
  ...
  if [[ -f "/run/dhclient-${ifc}.pid" ]]; then
    dhclient -4 -r -pf "/run/dhclient-${ifc}.pid" "$ifc" >/dev/null 2>&1 || true
...
dhclient_start() {
  local ifc="$1"
  dhclient_stop "$ifc"
  dhclient -4 -v -pf "/run/dhclient-${ifc}.pid" -lf "/var/lib/dhcp/dhclient.${ifc}.leases" "$ifc" >/dev/null 2>&1 || true
}
